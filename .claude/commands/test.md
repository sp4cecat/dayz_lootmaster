...
- `tests/server/history-*.test.js`, `tests/server/loot-cycle*.test.js`, `tests/server/be-guid.test.js` — recorded history store (in-memory `node:sqlite`), the pure loot-cycle scorer (cycler / dumper / base-triage / legacy-mod / hysteresis fixtures), the runner's refusals (lossy window, silent row, manual rung, cooldown), policy validation, webhook embeds, BE GUID byte order
