...
export default function ExpansionLogModal({ onClose, selectedProfileId }) {
  ...
  const refineAndDownload = () => {
    try {
      ...
      const hdrMatch = /^ExpansionLog started on\s+(\d{4}-\d{2}-\d{2})\s+at\s+(\d{1,2}:\d{2}:\d{2})/.exec(header);
  ...
  const fetchExpansionLog = async () => {
    ...
    try {
      ...
      const filename = `${formatForFilename(sNorm.toDate())}_to_${formatForFilename(eNorm.toDate())}${filterPart}.log`;
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Expansion log records">
      <div className="modal adm-modal">
        <div className="modal-header">
          <h3>Expansion Log records</h3>
        ...
        <div className="modal-body">
          ...
          <div style={{ marginTop: 12 }}>
            <button className="btn primary" onClick={fetchExpansionLog} disabled={busy}>
              {busy ? 'Fetching…' : 'Fetch'}
            </button>
          </div>
        </div>
