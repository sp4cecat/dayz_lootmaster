...
export interface SpawnableEntryLocation {
  entry: any;
  group: string;
  file: string;
}

export function findSpawnableEntryForType(
  spawnableTypesByGroup: any,
  group: string | undefined,
  typeName: string
): SpawnableEntryLocation | null {
  const target = String(typeName || '').toLowerCase();
  if (!target) return null;
  const findInGroup = (groupKey: string | undefined): SpawnableEntryLocation | null => {
